			...
			<div className="flex h-64 w-full flex-col items-start justify-center border-b-[1px] border-gray-100 px-5 text-black hover:cursor-default hover:border-gray-100 hover:bg-gray-100">
				<div className="flex h-[210px] flex-row">
					<div className="min-w-[77px] max-w-[140px] md:min-w-[140px]">
						{poster_path ? (
							<img src={`https://image.tmdb.org/t/p/w600_and_h900_bestv2${poster_path}`} width="140px" alt="movie poster" />
						) : (
					...
					<div className="flex flex-col justify-start pl-6 text-left">
						...
						<div className="mb-1 block">
							{release_date ? <h4 className="text-md mr-2 inline-block font-bold">{release_date.slice(0, 4)}</h4> : <></>}
...
export const SearchBar: React.FunctionComponent<SearchProps> = ({ searchBarOpen, setSearchBarOpen, openAddToListDialog }) => {
	...
	return (
		<>
			<div className={`ml-4 flex flex-row lg:ml-10 ${searchBarOpen ? '' : 'hidden'} sm:hidden`} id="close-search-bar">
