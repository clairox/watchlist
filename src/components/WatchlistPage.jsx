		...
		async (itemId) => {
			axios
				...
				.then(async (res) => {
					...
					setWatchlistItems(
						watchlistItems.filter((data) => data.id !== itemId)
					);
				});
		},
		[id, watchlistItems]
		...
		<div className="mt-6">
			<div className="mx-7 flex h-[37px] max-w-[1500px] flex-row 2xl:mx-auto">
				...
				<div className="flex flex-row">
					<div
						className={`w-10 hover:cursor-pointer md:w-12 md:text-lg ${
							editMode ? "text-green-500" : "text-gray-100"
						}`}
					>
						<FontAwesomeIcon
							icon={faPencil}
					...
					<div
						className={`w-10 ${
							_default
								? "text-gray-500"
								: "text-gray-100 hover:cursor-pointer hover:text-red-500"
						} md:w-12 md:text-lg`}
					>
						<FontAwesomeIcon
							icon={faTrashCan}
